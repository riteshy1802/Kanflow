...
const SidebarProjectsSkeleton: React.FC<BoardsSkeletonProps> = ({ count = 3 }) => {
  return (
    <div className="space-y-1 mt-1">
        <Skeleton data-testid="skeleton" className="h-8 w-full rounded-md bg-gray-600/50"/>
        {Array.from({ length: count }).map((_, idx) => (
            <div key={idx} className="w-full flex items-center gap-2">
            <div className="w-full flex items-center gap-2">
                    <p className="text-white">-</p>
                    <Skeleton data-testid="skeleton" className="h-7 w-[90%] rounded-md bg-gray-600/50" />
                </div>
            </div>
        ))}
    </div>
